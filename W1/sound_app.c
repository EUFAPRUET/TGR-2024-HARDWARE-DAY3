...
int main(int argc, char *argv[]) {
    ...
    char file_path[100];
    const char *url = "http://192.168.1.222:5000/api/upload";
    ...
    while (1) {
        ...
        if (feature > THRESHOLD && !wav_file) {
            ...
            log_audio_event(audio_db, "recording_started");
        }
        ...
        if (wav_file && feature > THRESHOLD) {
            ...
            sample_count += FRAMES;
            flag = 1;
            log_audio_event(audio_db, "freature_process_successfully");
            ...
            fclose(wav_file);
            wav_file = NULL;
            log_audio_event(audio_db, "recording_ended");
