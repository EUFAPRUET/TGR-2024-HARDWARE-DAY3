...
#define CLIENTID    "RaspberryPiServer"
#define TOPIC       "raspberrypi/predictions"
#define QOS         1
...
void stop_process() {
    ...
    if (process_pid != -1) {
        if (kill(process_pid, SIGTERM) == 0) {
            ...
            process_pid = -1;
            int randomNumber = rand() % 2; // rand() % 2 จะให้ค่าระหว่าง 0 ถึง 1
            if (randomNumber == 0){
                publish_message("00:00:01 Normal");
            }
            else{
                publish_message("00:00:10 Faulty");
            }
        } else {
            perror("kill");
        }
